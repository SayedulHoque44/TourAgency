...
export default class RestaurantController {
    ...
    try {
      ...
      await Promise.resolve().then(async () => {
        ...
        await LocationModel.findByIdAndUpdate(id, {
          // location er bitore resturant array er bitore created resuturant er id dukai dicci
          $addToSet: {
  ...
  public async updateRestaurant(req: Request, res: Response): Promise<void> {
    try {
      ...
      //   na paile restuanrant
      const findRestaurant = await RestaurantModel.findById(rid);
      if (!findRestaurant) {
        res.status(404).json({ message: 'Restaurant not found' });
        return;
      }
